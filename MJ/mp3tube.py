...
class Youtube_mp3:
    def youtube_mp3(self,pagedown=0):    # youtube_mp3(페이지 내리기 갯수)       
         # youtube 주소
...
class csv_file:
    def csvfile(self,save,filename):  # csvsave(저장경로, 파일명)
        import csv
...
class mp3_output:
    def mp3output(self,url1,indexnum=0):      # mp3output(변환 주소,남은 url number)   
...
class mp3tube(Youtube_mp3,csv_file,mp3_output):
    def guide():
            print("변수=mp3tube()")
            print("변수.Youtube_mp3(페이지 내리기)")
            print("변수.csv_file(저장경로, 파일명)")
            print("변수.mp3_output(변환 주소,남은 url number)")
